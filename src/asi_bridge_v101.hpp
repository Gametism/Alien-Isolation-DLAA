...
namespace asi_bridge_v101
{
    constexpr uint32_t MAGIC = 0x41314942u;
    constexpr uint32_t VERSION = 101u;
...
#pragma pack(pop)
    ...
    inline volatile LONG requested_sharpness_percent = 35;
    inline volatile LONG requested_sharpness_serial = 0;

    inline volatile LONG requested_render_preset = 11;
    inline volatile LONG requested_render_preset_serial = 0;

    inline void set_render_preset(LONG preset)
    {
        if (preset < 10 || preset > 13)
            preset = 11;

        InterlockedExchange(&requested_render_preset, preset);
        const LONG serial =
            InterlockedIncrement(&requested_render_preset_serial);

        if (st)
        {
            st->render_preset = preset;
            st->render_preset_serial = serial;
        }

        request_history_reset();

        logline(
            "BRIDGE render preset request preset=%ld serial=%ld",
            preset,
            serial);
    }
    ...
    inline void set_sharpness(float value)
    {
        ...
        if (value > 0.50f) value = 0.50f;

        LONG percent = static_cast<LONG>(value * 100.0f + 0.5f);
        ...
        InterlockedExchange(&requested_sharpness_percent, percent);
        const LONG serial = InterlockedIncrement(&requested_sharpness_serial);

        if (st)
        {
            st->sharpness_percent = percent;
            st->sharpness_serial = serial;
        }

        request_history_reset();
        logline(
            "BRIDGE sharpness request value=%.2f serial=%ld",
    ...
    {
        return static_cast<float>(requested_sharpness_percent) / 100.0f;
    }
        ...
        bool synchronized)
    {
        ...
        st->mode_serial=requested_mode_serial;
        st->auto_exposure=requested_auto_exposure;
        st->sharpness_percent=requested_sharpness_percent;
        st->sharpness_serial=requested_sharpness_serial;
        st->render_preset=requested_render_preset;
        st->render_preset_serial=requested_render_preset_serial;
